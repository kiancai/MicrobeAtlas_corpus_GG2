...
scripts/ResMicroDb/          # 398 项目 ASV → 跨项目 sample × genus
  ...
  07_attach_metadata.py            metadata parquet → anndata.obs 37 列（Database + Run 列 + 35 metadata；Run 既作 obs_names 又作列）
...
- 07 → `resmicrodb.gg2.genus.qc.with_meta.h5ad` 93,425 × 4,952（X 不变，obs 补 37 列 = Database + Run + 35 metadata）
...
- 待办：`06b_fix_metadata_errors`（ResMicroDb 部分列取值修错）— 暂时不影响下游使用

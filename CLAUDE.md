...
scripts/ResMicroDb/          # 398 项目 ASV → 跨项目 sample × genus
  ...
  06b_export_patches.R             从 ps.16s_0105_new7.rds 导出 Run 级 patch 映射 tsv（baseR + phyloseq）
  06b_fix_metadata_errors.py       应用 0105_new7 修正：5 项目 1,641 Run（Sample_Site/Phenotype/Phenotype_ID/cc/Is_Healthy） → .standardized.fixed.parquet
...
- 06b → `metadata_all.standardized.fixed.parquet` 135,746 × 36（应用 ps.16s_0105_new7 错标修正：1,641 Run / 5 列）
...
> 06b 三个 patch 详情（Sample_Site Nasal→Nasopharynx 1,257 / PRJNA801796 Influenza 细分 + Phenotype_ID 补齐 / PRJNA822681 + PRJNA824137 错标按 host.disease + real_group 同步改 cc/ih/Phenotype/Phenotype_ID）→ `rawdata/ResMicroDb/supplement data/CHANGES_0105_new7.md`
